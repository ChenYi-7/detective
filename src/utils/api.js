...
/**
 * 快速开工模式 - 识别桌面照片并生成2-5个整理线索
 */
export async function analyzeDeskPhoto(imageBase64) {
  const prompt = `你是一个侦探助手。用户拍了一张桌面的照片，请分析照片内容，找出2-5个需要整理的物品/区域，每个对应一个可执行的小任务（线索）。
...
/**
 * 委托案件模式 - 根据案件名称和材料生成任务树
 */
export async function generateTaskTree(caseName, materials) {
  const prompt = `你是一个侦探助手。用户委托了一个案件，案件名称是：「${caseName}」

${materials ? `用户提供了以下佐证材料描述：${materials}` : '用户没有提供额外材料。'}

请为这个案件生成一个任务树（3-5个任务），每个任务应该是2-5分钟可完成的小步骤。

请严格按以下JSON格式返回（不要返回其他内容）：
{
  "case_analysis": "对案件的简要分析（1-2句话）",
  "tasks": [
    {
      "task_title": "任务一标题",
      "task_desc": "任务详细说明",
